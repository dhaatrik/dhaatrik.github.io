...
<Scrollytell id="deltav-lab-insertion-dossier" stepCount={4}>
  ...
  <div slot="step-2">
    ...
    <p class="text-sm text-slate-600 dark:text-slate-400">
      Dynamic pressure peaks when atmosphere is still thick and velocity is climbing. DeltaV Lab exposes `DYNAMIC_PRESSURE` as a flight-computer variable — the worker integrates drag from $q = \frac{1}{2}\rho v^2$ at 50&nbsp;Hz, not a cinematic throttle hack.
    </p>
    <p class="mt-2 font-mono text-xs text-slate-500">
...
- $I_\{sp\}$ is specific impulse (seconds) — DeltaV Lab uses separate sea-level and vacuum values per engine.
