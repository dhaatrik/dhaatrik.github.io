...
6. **Sitemap & Icons** — Flattened `sitemap.xml` + `sitemap-index.xml` via `@astrojs/sitemap` with `<lastmod>` timestamps; PNG favicons (`48x48`, `192x192`) + `apple-touch-icon.png` in `BaseHead`.
